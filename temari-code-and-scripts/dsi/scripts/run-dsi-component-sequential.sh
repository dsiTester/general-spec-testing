...
if [ $# -lt 3 ]; then
    echo "Usage: $0 IDENT PROJECT_FILE SMAP"
    exit
fi
...
function run_dsi_plus() {
    ...
    echo "Running DSI in sequence started at: `date +%Y-%m-%d-%H-%M-%S`" | tee -a ${RUN_INFO_FILE}
    for input in $( cat ${all_configs_file} | sed "s/$/@${mode}@${MINING_OP}/g" ); do
        bash ${SCRIPT_DIR}/run-dsi-plus-parallel.sh ${input}
    done
    echo "Running DSI in sequence ended at: `date +%Y-%m-%d-%H-%M-%S`" | tee -a ${RUN_INFO_FILE}
