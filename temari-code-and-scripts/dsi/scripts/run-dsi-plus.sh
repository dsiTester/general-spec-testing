...
if [ $# -lt 4 -o $# -gt 6 ]; then
    ...
    echo "where OPTION is one of dsiAllGranularities, allGranularities, allTests, testClasses, testMethods, baseLineRun"
...
valid_options="allGranularities|allTests|testClasses|testMethods|dsiAllGranularities|baseLineRun"
...
function surefire_test() {
    ...
    buildDir=""
    if [[ ${OPTION} == *"NoDSI"* ]]; then
	echo "surefire_test: NoDSI option/testMethods option, not adding buildDirectory to args..."
    else
...
function main() {
	...
	echo "Setup ended at: `date +%Y-%m-%d-%H-%M-%S`" | tee -a ${RUN_INFO_FILE}
        ...
        (
            ...
            if [ "${option}" == "dsiPlus-baseLineRun" ]; then
                run_baseline ${RESULT_DIR}/${name}/${option}/logs
            else
