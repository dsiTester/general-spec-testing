...
//	public final static boolean onlyName = true;
	public final static boolean onlyName = false;
//	public final static boolean writeParameterValues = false;
	public final static boolean writeParameterValues = true;
...
//							methodName += "{" + String.join("", abstractedArguments) + "}";
					...
					if (primitiveTraces == null && nullTraces == null) {
						...
						if (primitiveEntry != null) {
							...
							// I think this is where we get the :FALSE from. removing...
//							try {
...
//			LoggingUtils.logWarnAtMostOnce(logger, "superClass=" + clazz.getSuperclass() +" of=" + clazz);
	...
	public static void writeTracesToFile(List<String> methodEventNames, Class<?> clazz, String dirName) throws IOException {
		if (!classTracesWriter.containsKey(clazz)) {
			...
			logger.warn("ADDING KEY " + dirName +  Properties.CONFIGURATION_ID +  "_" + clazzFileName + ".traces TO CLASS TRACES WRITER" );
		}
		
		BufferedWriter bufferedWriter = classTracesWriter.get(clazz);
//		LoggingUtils.logWarnAtMostOnce(logger, "writeTracesToFile: write to " +dirName + clazz.getName() + ".traces");
		logger.warn("writeTracesToFile: write to " +dirName + clazz.getName() + ".traces");
		bufferedWriter.write("<START> ");
		bufferedWriter.write(String.join(" ", methodEventNames));
//		logger.warn("WRITING STRING " + String.join(" ", methodEventNames) + " TO FILE ");
...
//		logger.warn("methods of claass: " + methodsOfClazz);

		for (int i = 0; i < results.size(); i++) {
			ExecutionResult result = results.get(i);
			TestCase test = tests.get(i);
//			logger.warn("TestCase test:\n"  + test);
...
//				logger.warn("stmt : " + stmt + " classOfVar: " + classOfVar + " objectToTrace: " + objectToTrace);
...
//
//				System.out.println("Adding methodName " + methodName + " and objectToTrace: " + objectToTrace +" to tracesOfVar");
...
//			    			.map(event -> event.getName())
		    			...
		    			clazzMethods.addAll(
		    					Arrays.asList(clazz.getConstructors()).stream()
										.map(event -> event.getName())
		    					.collect(Collectors.toSet()));
//						System.out.println("clazzMethods of class " + clazz.getName() + ": " + clazzMethods);
...
//						logger.warn("original traces: " + traces);
		    			...
		    			String trace = 	String.join(" ", filtered);
							boolean inDebugging = true;
						   if (inDebugging) {
							   logger.warn("Trace to write: " + "<START> " + trace + " <END>\n");
						   }
...
//			logger.warn("END tracesOfVar: " + tracesOfVar);
		}
//		logger.warn("END: methods of claass: " + methodsOfClazz);
	}
...
//		boolean retVal = clazzMethods.contains(eventStripped);
		logger.warn("in classMethodsInclude. clazzMethods: " + clazzMethods + " event: " + event);
		boolean retVal = clazzMethods.contains(event);
