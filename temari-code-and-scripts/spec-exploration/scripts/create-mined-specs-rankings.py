    ...
    with open(os.path.join(dirname, filename), "r") as f:
        for line in f:
            ...
            if len(sline) == 3:
                ...
                states.add(sline[1])
                lbl = sline[2]
                if not( lbl == "<BEGIN>" or lbl == "<END>" or lbl == "<START>" or lbl == "<IO-LEAK>" ):
                    letters.add(lbl)
                num_transitions += 1
...
def get_edge_pair_stats(edge_pair_analysis_dir, spec_data):
    categories = [ "true-spec", "spurious-spec", "no-break-pass", "no-mined-spec" ]

    edge_pair_file = os.path.join(edge_pair_analysis_dir, spec_data["project"], spec_data["spec-id"] + ".csv")
    if not os.path.isfile(edge_pair_file):
        return { cat : -1 for cat in categories }
    counts = { cat : 0 for cat in categories }
    edge_pair_analysis = pd.read_csv(edge_pair_file)
    num_edge_pairs = 0
    for _, row in edge_pair_analysis.iterrows():
        num_edge_pairs += 1
        for cat in categories:
            if (cat in row["verdict"]):
                counts[cat] += 1
    counts["total-edge-pairs"] = num_edge_pairs

    return counts

def collect_data(miner_name, fsms_dir, edge_pair_analysis_dir):
    spec_lst = []
    for project in os.listdir(fsms_dir):
        ...
        for filename in os.listdir(project_dir):
            ...
            spec_dict.update(get_edge_pair_stats(edge_pair_analysis_dir, spec_dict))
...
def wrapper(miner_name, fsms_dir, edge_pair_analysis_dir):
    data = collect_data(miner_name, fsms_dir, edge_pair_analysis_dir)
    print(data[0])
    filtered_data = data
    ...
    keys = [ "project", "spec-id", "states", "transitions", "letters", "true-spec", "spurious-spec", "no-break-pass", "no-mined-spec" , "total-edge-pairs" ]
    lst_for_out = [ {key : key for key in keys } ]
    lst_for_out += sorted_data
    output_to_csv(keys, lst_for_out, out_dir, miner_name + "-rankings.csv")

if __name__ == '__main__':
    if len(sys.argv) != 4:
        print("Usage: " + sys.argv[0] + " miner_name fsms_dir edge_pair_analysis_dir")
        sys.exit(1)
