if [ $# -lt 1 ]; then
    echo "USAGE: $0 PML_FILE SETUP_SPIN_OPT"
...
function postprocess() {
    ...
    cat ${LOGS_DIR}/gol-run-spin | grep -v ^$ | grep -v ^real | grep -v ^user | grep -v ^sys > ${FILES_DIR}/outcomes.txt
}

function main() {
    create_constraints
    echo "[ LOG ] Running spin"
    ( time bash ${SCRIPT_DIR}/run-spin-on-constraints.sh ${PML_FILE} ${FILES_DIR}/constraints.txt ${SPIN_DIR} ${LOGS_DIR} ) &> ${LOGS_DIR}/gol-run-spin
    postprocess
    # bash s.sh fsa.pml
