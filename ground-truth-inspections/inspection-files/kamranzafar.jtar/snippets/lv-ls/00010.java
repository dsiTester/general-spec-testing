...
public void testAppend() throws IOException {//lv
    ...
    tar.putNextEntry(new TarEntry(TestUtils.writeStringToFile("a", new File(inDir, "afile")), "afile"));//calls method b
    copyFileToStream(new File(inDir, "afile"), tar);
    tar.close();

    tar = new TarOutputStream(new File(dir, "tar.tar"), true);
    tar.putNextEntry(new TarEntry(TestUtils.writeStringToFile("b", new File(inDir, "bfile")), "bfile"));//calls method b
    copyFileToStream(new File(inDir, "bfile"), tar);
    tar.putNextEntry(new TarEntry(TestUtils.writeStringToFile("c", new File(inDir, "cfile")), "cfile"));//calls method b
...
private void assertInEqualsOut() throws UnsupportedEncodingException, FileNotFoundException, IOException {
    ...
    for (File in : inDir.listFiles()) {
        assertEquals(TestUtils.readFile(in), TestUtils.readFile(new File(outDir, in.getName())));//we fail this assertion
    }
}
