...
private <T> MethodsStringConverter<T> findFromStringMethod(Class<T> cls, Method toString, boolean searchSuperclasses) {//method b
    ...
    if (searchSuperclasses) {//boolean that happened to be false during trace collection
        for (Class<?> loopIfc : eliminateEnumSubclass(cls).getInterfaces()) {//method a, being called within method b
